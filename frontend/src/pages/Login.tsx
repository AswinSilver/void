...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-[#030712] flex items-center justify-center px-4 relative overflow-hidden">
      {/* Background glows */}
      <div className="absolute top-[-20%] left-[-10%] w-[500px] h-[500px] rounded-full bg-cyan-500/5 blur-3xl pointer-events-none" />
      <div className="absolute bottom-[-20%] right-[-10%] w-[500px] h-[500px] rounded-full bg-purple-500/5 blur-3xl pointer-events-none" />
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Sign in to VOID</h1>
          <p className="text-slate-400 text-sm mt-2">Phishing investigation platform</p>
        </div>

        <div className="glass rounded-2xl p-8 border border-white/8">
          {/* Demo credentials hint */}
          <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-cyan-500/8 border border-cyan-500/20 mb-6">
            <AlertCircle size={14} className="text-cyan-400 mt-0.5 flex-shrink-0" />
            <p className="text-xs text-cyan-300/80">
              Demo: <span className="mono font-medium">demo@void.security</span> / <span className="mono font-medium">password123</span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            <button className="flex items-center justify-center gap-2 py-2 rounded-xl border border-white/10 hover:bg-white/5 text-sm text-slate-300 transition-all duration-200">
              <svg width="16" height="16" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
              Google
            </button>
            <button className="flex items-center justify-center gap-2 py-2 rounded-xl border border-white/10 hover:bg-white/5 text-sm text-slate-300 transition-all duration-200">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="#0078d4"><path d="M11.4 24H0V12.6L11.4 24zM12.6 24H24V12.6L12.6 24zM0 11.4V0h11.4L0 11.4zM12.6 0H24v11.4L12.6 0z"/></svg>
              Microsoft
            </button>
          </div>
        </div>
